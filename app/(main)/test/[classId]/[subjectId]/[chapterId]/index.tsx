...
export default function SpecificTestScreen() {
  ...
  const { startTest, answerQuestion, goToNextQuestion, goToPreviousQuestion, getCurrentQuestion, submitTest, cancelTest, generateSpecificTestQuestions, currentSession } = useTestStore();
  const { getChapter } = useCurriculumStore();
  ...
  useEffect(() => {
    if (chapterId) {
      ...
      const questions = generateSpecificTestQuestions(
        classId as string,
        subjectId as string,
        chapterId as string
      );
